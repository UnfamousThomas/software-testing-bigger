...
public class SalesSystemUI extends Application {
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        teamTab.setContent(loadControlsForTeamTab("TeamTab.fxml", new TeamController(), "application"));
    ...
    private Node loadControlsForTeamTab(String fxml, Initializable controller, String fileName) throws IOException {
        URL resource = getClass().getResource(fxml);
        if (resource == null)
            throw new IllegalArgumentException(fxml + " not found");

        FXMLLoader fxmlLoader = new FXMLLoader(resource);
        fxmlLoader.setResources(ResourceBundle.getBundle(fileName));
        fxmlLoader.setController(controller);
        return fxmlLoader.load();
    }
}
